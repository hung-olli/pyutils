...
def train_test_split_fold_by_keys(**config):
    ...
    if valid_fold is None:
        s = list(config['train_df'][fold_key])
        s = list(set(s))
        n_valid = int(len(s) * config.get('valid_rate', 0.2))
        n_valid = n_valid if n_valid > 1 else 1
        valid_fold = [random.choice(s) for _ in range(n_valid)]
